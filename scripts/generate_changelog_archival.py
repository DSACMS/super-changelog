"""
Archival Changelog Data Generation Script
"""
...
def main() -> None:
    """Main function to generate archival changelog data."""
    ...
    if not start_raw or not start_raw.strip():
        print(
            ...
            "python scripts/generate_changelog_archival.py <start_date> [end_date]"
        )
        sys.exit(1)
    ...
    filename = os.path.join(output_dir, f"archival_changelog_{start_date}_to_{end_date}.json")

    print(f"Generating archival changelog data for {org_name} from {start_date} to {end_date}...")
    ...
    saved = gen.get_and_save_archive_data(org_name=org_name)

    if saved:
        print("-" * 60)
        print(f"archival changelog data generated and saved to {filename}")
    else:
        print("Failed to generate archival changelog data.")
        sys.exit(1)
